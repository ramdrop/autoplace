...
# private library
import nuscene_seqnet as dataset
...
def update_opt_from_json(flag_file, opt):
    restore_var = [
        'net', 'seqLen', 'num_clusters', 'output_dim', 'structDir', 'imgDir', 'lrStep', 'lrGamma', 'weightDecay', 'momentum',
         'num_clusters', 'optim', 'margin', 'seed', 'patience', 'w'
    ]
...
def train(opt, seed_worker=None, trial=None):
    ...
    # ---------------------------------- 2B. create new training --------------------------------- #
        ...
        for file in [__file__, 'nuscene_seqnet.py', 'networks/{}.py'.format(opt.net)]:
    ...
    whole_training_data_loader = DataLoader(dataset=whole_train_set, num_workers=opt.threads, batch_size=opt.cacheBatchSize, shuffle=False, pin_memory=cuda)
    ...
    for epoch in range(opt.start_epoch + 1, opt.nEpochs + 1):
        ...
        print('training..')
        model.train()
        training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize, shuffle=True, collate_fn=dataset.collate_fn, pin_memory=cuda)  # pin_memory=cuda ?
        for iteration, (query, positives, negatives, negCounts, indices) in enumerate(tqdm.tqdm(training_data_loader, ncols=40), startIter):
            ...
            B, L, D, _ = query.shape                                        # ([8, 3, 200, 200])
        ...
        if (epoch % opt.evalEvery) == 0:
            ...
            is_best = current_recalls[1] > best_score
            if is_best:
                not_improved = 0
                best_score = current_recalls[1]
                torch.save({
                    'epoch': epoch,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgDir', type=str, default='dataset/7n5s_xy11/seqnet_feat3', help='Path for images.')
    ...
    parser.add_argument('--net', type=str, default='seqnet', help='network')
    ...
    parser.add_argument('--cGPU', type=int, default=2, help='core of GPU to use.')  # modified
    ...
    parser.add_argument('--logsPath', type=str, default='./logs_seqnet', help='Path to save runs to.')
    ...
    parser.add_argument('--encoder_dim', type=int, default=4096, help='Number of feature dimension. Default=512')
    parser.add_argument('--output_dim', type=int, default=4096, help='Number of feature dimension. Default=512')
    parser.add_argument('--w', type=int, default=2, help='Number of feature dimension. Default=2')
